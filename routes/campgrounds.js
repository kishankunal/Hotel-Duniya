...
//var geocoder = require('geocoder');   //will show location as string later we'll implement it using google map
...
//add link for the hotel website
...
router.get("/:id", function(req, res){
    ...
    Campground.findById(req.params.id).populate("comments").exec(function(err, foundCampground){
        ...
        Comment.find().where("campid").equals(foundCampground._id.toString()).exec((err1,comment)=>{
          if(err1){
        ...
        res.render("campgrounds/show", {campground: foundCampground , comment : comment});
      });
    });
});
